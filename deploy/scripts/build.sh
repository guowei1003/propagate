...
docker build -f "$ROOT_DIR/deploy/docker/api.Dockerfile" -t propagate/api:latest "$ROOT_DIR"

echo "[3/3] Building Frontend image..."
docker build -f "$ROOT_DIR/deploy/docker/frontend.Dockerfile" -t propagate/frontend:latest "$ROOT_DIR"
